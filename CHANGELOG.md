...
### 🔒 Repo Cleanup & Public Release

**Repository:**
- Renamed to `runsodin` (was `printfarm-scheduler`)
- Git history orphaned — single clean commit, no leaked secrets
- 47 development/patch scripts removed from tracking
- `generate_license.py` removed (license generator stays private)
- `.gitignore` updated with comprehensive dev artifact patterns
- Tracked files reduced from 175 to 128

**QA Suite (1,031 tests):**
- Phase 1: RBAC — 863 tests covering all 164 API endpoints (811 pass, 51 skip for unimplemented)
- Phase 2: Security — 31 tests (auth bypass, injection, session management) — all pass
- Phase 3: Features — 62 tests (scheduling, orders, inventory) — 58 pass, 4 skip for hardware
- Phase 4: Conditional UI — 43 Playwright tests (role/license-dependent visibility)
- Phase 5: E2E Workflows — 32 Playwright tests (full user journeys)
- Zero security failures

**Landing Page (runsodin.com):**
- Updated GitHub URLs to `runsodin`
- Beta badges on Klipper, PrusaLink, Elegoo printer sections
- Test count badge (1,031 automated tests)
