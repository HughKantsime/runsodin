...
function PricingBreakdown({ costs, margin, quantity }) {
  ...
  return (
    <div className="bg-farm-900 rounded-xl border border-farm-800 p-3 md:p-4 lg:sticky lg:top-4">
...
export default function CalculatorPage() {
  ...
  const costs = { material: materialCost, labor: laborCost, electricity: electricityCost, depreciation: depreciationCost, packaging: packagingCost, failureRate, failure: failureCost, overhead: overheadCost, other: otherCosts }
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-farm-900 rounded-xl border border-farm-800 p-3 md:p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              <div>
                ...
                <select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} className="w-full bg-farm-800 border border-farm-700 rounded-lg px-3 py-2 text-sm">
                  <option value="">-- Manual Entry --</option>
                  {modelsData?.map(model => (<option key={model.id} value={model.id}>{model.name}</option>))}
                </select>
              </div>
          ...
          <CostSection title="Material Costs" icon={Package} cost={materialCost}>
            <div className="grid grid-cols-3 gap-2 md:gap-3">
              ...
              <InputField label="Spool Cost" value={spoolCost} onChange={setSpoolCost} suffix="$" />
              <InputField label="Spool Weight" value={spoolWeight} onChange={setSpoolWeight} suffix="g" />
            </div>
          ...
          <CostSection title="Labor Costs" icon={Clock} cost={laborCost}>
            <InputField label="Your Hourly Rate" value={hourlyRate} onChange={setHourlyRate} suffix="$/hr" />
            <div className="grid grid-cols-3 gap-2 md:gap-3">
              <InputField label="Post-Processing" value={postProcessingMin} onChange={setPostProcessingMin} suffix="min" />
              <InputField label="Packing Time" value={packingMin} onChange={setPackingMin} suffix="min" />
              <InputField label="Customer Support" value={supportMin} onChange={setSupportMin} suffix="min" />
            </div>
            <div className="text-xs text-farm-500">Total time: {postProcessingMin + packingMin + supportMin} min ({laborHours.toFixed(2)} hours)</div>
          </CostSection>

          <CostSection title="Electricity" icon={Zap} cost={electricityCost}>
            <div className="grid grid-cols-3 gap-2 md:gap-3">
              <InputField label="Electricity Rate" value={electricityRate} onChange={setElectricityRate} suffix="$/kWh" />
              <InputField label="Printer Wattage" value={printerWattage} onChange={setPrinterWattage} suffix="W" />
              <InputField label="Print Time" value={printTimeHours} onChange={setPrintTimeHours} suffix="hrs" />
            </div>
            <div className="text-xs text-farm-500">Energy: {((printerWattage / 1000) * printTimeHours).toFixed(3)} kWh</div>
          </CostSection>

          <CostSection title="Printer Depreciation" icon={Printer} cost={depreciationCost}>
            <div className="grid grid-cols-2 gap-2 md:gap-3">
              <InputField label="Printer Cost" value={printerCost} onChange={setPrinterCost} suffix="$" />
              <InputField label="Expected Lifespan" value={printerLifespan} onChange={setPrinterLifespan} suffix="hrs" />
            </div>
            <div className="text-xs text-farm-500">Rate: ${(printerCost / printerLifespan).toFixed(4)}/hr</div>
          </CostSection>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <CostSection title="Packaging" icon={Box} cost={packagingCost}>
              <InputField label="Box/Packaging Cost" value={packagingCost} onChange={setPackagingCost} suffix="$" />
            </CostSection>
            <CostSection title="Failure Rate" icon={AlertTriangle} cost={failureCost}>
              <InputField label="Failure Rate" value={failureRate} onChange={setFailureRate} suffix="%" />
              <div className="text-xs text-farm-500">Buffer for failed prints</div>
            </CostSection>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <CostSection title="Rent & Overhead" icon={DollarSign} cost={overheadCost}>
              <InputField label="Monthly Rent/Overhead" value={monthlyRent} onChange={setMonthlyRent} suffix="$/mo" />
              <InputField label="Parts Per Month" value={partsPerMonth} onChange={setPartsPerMonth} suffix="parts" />
            </CostSection>
            <CostSection title="Other Costs" icon={Package} cost={otherCosts}>
              <InputField label="Screws, inserts, etc." value={otherCosts} onChange={setOtherCosts} suffix="$" />
            </CostSection>
          </div>
          ...
          <div className="bg-farm-900 rounded-xl border border-farm-800 p-3 md:p-4">
            <div className="flex items-center gap-2 mb-3 md:mb-4">
              ...
              <h3 className="font-medium text-sm">Manufacturing Margin</h3>
            </div>
            <div className="flex items-center gap-4">
              <input type="range" min="0" max="200" value={margin} onChange={(e) => setMargin(parseInt(e.target.value))} className="flex-1 accent-print-500" />
              <div className="w-20">
                <InputField label="" value={margin} onChange={setMargin} suffix="%" />
              </div>
            </div>
          </div>
        </div>
