...
test-mutation-workflows: ## Verify publication/promotion identity, receipt, and source policies
	$(PYTHON) -m pytest tests/test_release_control/test_mutation_workflows.py -q --tb=short -o xfail_strict=true

test-registry-contract: ## Exercise exact release CLI against a disposable local OCI registry
	$(PYTHON) -m pytest tests/test_release_control/test_registry_integration.py -q --tb=short -o xfail_strict=true
