.PHONY: build test test-contracts test-contracts-structured test-candidate test-database-parity test-hardware-certification test-installer-isolation test-edu-sandbox-contracts test-edu-sandbox edu-sandbox-prepare edu-sandbox-request-license edu-sandbox-activate edu-sandbox-status edu-sandbox-reset edu-sandbox-expire edu-sandbox-reconcile edu-sandbox-purge edu-sandbox-certify test-edu test-edu-privacy test-edu-backup test-edu-hardware test-edu-load test-edu-accessibility test-edu-readiness test-telemetry-contracts-structured test-telemetry-v2-smoke-structured verify-edu-live verify-backup test-security test-e2e test-coverage scan security security-structured security-operational security-audit security-secrets security-sast security-docker release-control-local-gate trusted-validation-gate test-practical-evidence test-promotion-workflow practical-evidence verify-practical-evidence verify bump release logs shell tokens help
...
test-promotion-workflow: ## Run the exact 33-case authenticated eligibility contract
	@mkdir -p artifacts/release-control-local
	$(PYTHON) -m pytest tests/test_release_control/test_promotion_workflow.py -q --tb=short -o xfail_strict=true --junitxml=artifacts/release-control-local/promotion-workflow-junit.xml
	$(PYTHON) -c 'from collections import Counter; from xml.etree import ElementTree as E; p="artifacts/release-control-local/promotion-workflow-junit.xml"; c=list(E.parse(p).getroot().iter("testcase")); e=[*(f"test_PI{i:02d}" for i in range(1,11)),*(f"test_PO{i:02d}" for i in range(1,11)),*(f"test_PD{i:02d}" for i in range(1,7)),*(f"test_PF{i:02d}" for i in range(1,3)),"test_PR01",*(f"test_PW{i:02d}" for i in range(1,5))]; a=[next((x for x in e if t.get("name","").startswith(x)),"") for t in c]; assert len(c)==33 and Counter(a)==Counter(e) and all(t.find("failure") is None and t.find("error") is None and t.find("skipped") is None for t in c), (len(c),a)'
