...
from helpers import auth_headers as _auth_headers, login as _shared_login
...
class APIClient:
    ...
    def _login(self, username, password):
        ...
        if cache_key in _TOKEN_CACHE:
            ...
            return
        token = _shared_login(self.base_url, username, password, retries=4, timeout=20)
        if token:
            self.token = token
    ...
    def _headers(self):
        return _auth_headers(self.token) if self.token else {}
...
def seed_data(api):
    ...
    if resp.status_code == 200:
        ...
        if printers:
            ...
            data["printer_nickname"] = printers[0].get("nickname")
...
@pytest.fixture(scope="session")
def audit_printer(api, seed_data):
    """
    Ensure the audit sweep has a printer to target for route rendering.
    Reuses an existing printer when possible, otherwise creates a temporary one.
    """
    if "printer_id" in seed_data:
        yield {
            "id": seed_data["printer_id"],
            "name": seed_data.get("printer_name", "Audit Printer"),
            "nickname": seed_data.get("printer_nickname"),
            "created": False,
        }
        return

    name = f"ODIN Audit Printer {uuid.uuid4().hex[:6]}"
    resp = api.post("/api/printers", json={
        "name": name,
        "model": "Bambu Lab X1C",
        "api_type": "bambu",
        "slot_count": 4,
    ...
    if resp.status_code not in (200, 201):
        pytest.skip(f"Could not provision an audit printer: {resp.status_code} {resp.text[:200]}")
    ...
    if printer["created"] and printer["id"]:
        api.delete(f"/api/printers/{printer['id']}")


@pytest.fixture(scope="session")
def state_printer(api):
    """
    Dedicated unmonitored printer for state injection tests.
    Always created fresh so no real monitor overwrites synthetic state.
    Cleaned up after the session.
    """
    name = f"Audit State Printer {uuid.uuid4().hex[:6]}"
    resp = api.post("/api/printers", json={
        "name": name,
        "model": "Bambu Lab X1C",
        "api_type": "bambu",
    ...
    if resp.status_code not in (200, 201):
        pytest.skip(f"Could not create state injection printer: {resp.status_code} {resp.text[:200]}")
...
def _get_token(username, password, base_url=BASE_URL):
    ...
    if cache_key not in _TOKEN_CACHE:
        ...
        if len(_TOKEN_CACHE) > 0:
            time.sleep(1)
        token = _shared_login(base_url, username, password, retries=4, timeout=20)
...
def _login_via_token(browser_instance, username, password, base_url=BASE_URL, frontend_url=FRONTEND_URL):
    """
    Login via cached API token by seeding the session cookie the SPA expects.
    """
...
@pytest.fixture(scope="session")
def _admin_session(browser_instance):
    """Session-scoped admin context + page — one login for all tests."""
...
@pytest.fixture
def admin_page(_admin_session):
    """Reuse the session-scoped admin page. Navigates to root between tests."""
    _ctx, pg = _admin_session
    # Reset to a neutral page between tests to avoid stale state
    pg.goto(FRONTEND_URL, wait_until="domcontentloaded", timeout=10000)
    yield pg
