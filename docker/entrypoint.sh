...
conn.execute("""CREATE TABLE IF NOT EXISTS printer_profiles (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    created_by INTEGER REFERENCES users(id),
...
conn.execute("CREATE INDEX IF NOT EXISTS idx_printer_profiles_slicer ON printer_profiles(slicer)")
conn.execute("CREATE INDEX IF NOT EXISTS idx_printer_profiles_category ON printer_profiles(category)")
