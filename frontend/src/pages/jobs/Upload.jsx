      ...
      <label htmlFor="file-upload" className="cursor-pointer">
        <div className="flex flex-col items-center gap-3 md:gap-4">
          <div className={clsx('p-3 md:p-4 rounded-full', isDragging ? 'bg-[var(--brand-primary)]' : 'bg-[var(--brand-input-bg)]')}>
            <FileUp size={36} className={isDragging ? 'text-white' : 'text-[var(--brand-text-secondary)]'} />
          </div>
          <div>
            {isUploading ? (
              <>
                ...
                {uploadProgress != null && (
                  <div className="mt-3 w-48 mx-auto">
                    <div className="bg-[var(--brand-input-bg)] rounded-full h-2 overflow-hidden">
                      <div className="bg-[var(--brand-primary)] h-full rounded-full transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                    </div>
                    <p className="text-sm text-[var(--brand-text-secondary)] mt-1">{uploadProgress}%</p>
                  </div>
                )}
              </>
            ) : (
              <>
                ...
                <p className="text-[var(--brand-text-muted)] text-sm mt-1">or click to browse</p>
              </>
            )}
          </div>
        </div>
...
function FilamentBadge({ filament }) {
  return (
    <div className="flex items-center gap-2 bg-[var(--brand-input-bg)] rounded-md px-3 py-2">
      <div className="w-4 h-4 rounded-full border border-[var(--brand-text-muted)]" style={{ backgroundColor: filament.color }} />
      <span className="text-sm">{filament.type}</span>
      <span className="text-xs text-[var(--brand-text-muted)]">{filament.used_grams}g</span>
    </div>
  )
}
...
function UploadSuccess({ data, onUploadAnother, onViewLibrary, onScheduleNow, onUpdateObjects, submitForApproval, onSaveQuantity }) {
  ...
  return (
    <div className="bg-[var(--brand-card-bg)] rounded-md border border-[var(--brand-card-border)] overflow-hidden">
      <div className="flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-64 h-48 md:h-64 bg-[var(--brand-content-bg)] flex-shrink-0">
          {data.thumbnail_b64 ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-[var(--brand-text-muted)]">No preview</div>
          )}
        </div>
        ...
        <div className="flex-1 p-4 md:p-6">
          ...
          {data.filaments?.length > 0 && (
            <div className="mb-4">
              <p className="text-sm text-[var(--brand-text-muted)] mb-2">Filaments</p>
          ...
          {data.objects?.length > 0 && (
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm text-[var(--brand-text-muted)]">Objects on Plate</p>
                <p className="text-xs text-[var(--brand-text-secondary)]">
                  Sellable pieces: <span className="text-green-400 font-medium">{data.objects.filter(o => o.checked).length}</span>
                </p>
              </div>
              <div className="bg-[var(--brand-input-bg)]/50 rounded-md p-2 max-h-32 overflow-y-auto space-y-1">
                {data.objects.map((obj, i) => (
                  <label key={i} className="flex items-center gap-2 p-1.5 hover:bg-[var(--brand-card-border)]/50 rounded-md cursor-pointer">
                    <input
                      type="checkbox"
                      checked={obj.checked}
                      onChange={() => {
                      ...
                      className="w-4 h-4 rounded-md border-[var(--brand-text-muted)] bg-[var(--brand-card-border)] text-green-500 focus:ring-green-500"
                    />
                    <span className={`text-sm truncate ${obj.is_wipe_tower ? 'text-[var(--brand-text-muted)] italic' : ''}`}>
                      {obj.name}
          ...
          {!data.is_sliced && (
            <div className="bg-amber-900/30 border border-amber-700 rounded-md p-3 mb-4">
      ...
      <div className="flex justify-end gap-3 p-3 md:p-4 bg-[var(--brand-content-bg)] border-t border-[var(--brand-card-border)]">
        <button onClick={onUploadAnother} className="flex items-center gap-2 px-4 py-2 rounded-md border border-[var(--brand-card-border)] hover:bg-[var(--brand-input-bg)] transition-colors text-sm">
        ...
        {data.model_id && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-md bg-amber-600 hover:bg-amber-500 disabled:opacity-50 text-white font-medium transition-colors text-sm"
  ...
  return (
    <div className="mt-6 md:mt-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/models" className="text-sm text-[var(--brand-primary)] hover:text-[var(--brand-primary)]/80 transition-colors">View all models &rarr;</Link>
      </div>
    ...
    <div className="p-4 md:p-6">
      <div className="mb-6 md:mb-8">
        <div className="flex items-center gap-3">
          <UploadIcon className="text-[var(--brand-primary)]" size={24} />
          <div>
            <h1 className="text-xl md:text-2xl font-display font-bold">Upload Print File</h1>
            <p className="text-[var(--brand-text-muted)] text-sm mt-1">Upload .3mf files to add to your model library</p>
          </div>
        </div>
      </div>
      ...
      {uploadedFile ? (
        ...
        <>
          ...
          {uploadMutation.isError && (
            <div className="mt-4 bg-red-900/30 border border-red-700 rounded-md p-4">
