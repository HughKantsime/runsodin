...
function TVCameraStream({ cameraId }: { cameraId: number }) {
...
function TVPrinterCard({ printer }: { printer: any }) {
  ...
  return (
    <div
      data-testid="tv-printer-card"
      data-printer-id={printer.id}
      data-printer-name={printer.name}
      className="bg-[var(--brand-card-bg)] rounded-md border border-[var(--brand-card-border)] overflow-hidden flex flex-col"
    >
      {/* Camera thumbnail */}
      {hasCamera ? (
      ...
      <div className="p-3 flex-1 flex flex-col">
        ...
        {isPrinting && (
          <div className="mt-auto">
            {jobName && <p data-testid="tv-printer-job" className="text-sm text-[var(--brand-text-secondary)] truncate mb-2">{jobName}</p>}
            <div className="flex items-center justify-between mb-1.5">
              <span data-testid="tv-printer-progress" className="text-2xl font-bold font-mono text-[var(--status-completed)] tabular-nums">{progress}%</span>
...
export default function TVDashboard() {
  ...
  const resetAutoPage = React.useCallback(() => {
