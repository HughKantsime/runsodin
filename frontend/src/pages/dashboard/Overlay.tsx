  ...
  return (
    <div className={`h-screen w-screen flex overflow-hidden ${bg}`}>
      ...
      <div className={`${showCameraPanel ? 'flex-[3]' : 'flex-1 max-w-xl mx-auto'} flex flex-col p-4 gap-4 overflow-y-auto border-l ${border} ${panelBg}`}>
        {/* Printer name + status */}
        <div>
          <h1 data-testid="overlay-printer-name" className="text-lg font-semibold truncate">{data.printer_name}</h1>
          {data.model && <p className={`text-xs ${mutedText}`}>{data.model}</p>}
          <p data-testid="overlay-status" className={`text-sm font-medium mt-1 ${statusColor}`}>{statusLabel}</p>
        </div>
        ...
        {isPrinting && (
          <div className={`rounded-md p-3 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
            {data.job_name && (
              <p data-testid="overlay-job-name" className="text-sm font-medium truncate mb-2">{data.job_name}</p>
            )}
            ...
            <div className="flex justify-between mt-1 text-xs">
              <span data-testid="overlay-progress" className={`font-mono ${mutedText}`}>{Math.round(progress)}%</span>
