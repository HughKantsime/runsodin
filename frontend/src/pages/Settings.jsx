...
export default function Settings() {
  ...
  const saveSettings = useMutation({
    mutationFn: (data) => fetch('/api/config', {
      method: 'PUT',
  ...
  const testSpoolman = useMutation({
    mutationFn: () => fetch('/api/spoolman/test').then(r => r.json()),
  })
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-farm-500 mt-1">Configure your print farm</p>
      </div>

      {/* Spoolman Integration */}
      <div className="bg-farm-900 rounded-xl border border-farm-800 p-6 mb-6">
        <div className="flex items-center gap-3 mb-4">
          <Database size={20} className="text-print-400" />
          <h2 className="text-xl font-display font-semibold">Spoolman Integration</h2>
          {statsData?.spoolman_connected ? (
            <span className="flex items-center gap-1 text-sm text-green-400 bg-green-900/30 px-2 py-0.5 rounded">
              <CheckCircle size={14} /> Connected
            </span>
          ) : (
            <span className="flex items-center gap-1 text-sm text-farm-400 bg-farm-800 px-2 py-0.5 rounded">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex items-end">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-farm-700 hover:bg-farm-600 disabled:opacity-50 rounded-lg"
            >
              <Plug size={16} />
              Test Connection
            </button>
      ...
      <div className="bg-farm-900 rounded-xl border border-farm-800 p-6 mb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-display font-semibold">Scheduler Settings</h2>
        </div>
        <p className="text-sm text-farm-400 mb-4">
      ...
      <div className="bg-farm-900 rounded-xl border border-farm-800 p-6 mb-6">
        <div className="flex items-center gap-3 mb-4">
          <Database size={20} className="text-print-400" />
          <h2 className="text-xl font-display font-semibold">Database</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-farm-800 rounded-lg p-4">
            <div className="text-2xl font-bold">{statsData?.printers?.total || 0}</div>
            <div className="text-sm text-farm-400">Printers</div>
          </div>
          <div className="bg-farm-800 rounded-lg p-4">
            <div className="text-2xl font-bold">{statsData?.models || 0}</div>
            <div className="text-sm text-farm-400">Models</div>
          </div>
          <div className="bg-farm-800 rounded-lg p-4">
            <div className="text-2xl font-bold">{(statsData?.jobs?.pending || 0) + (statsData?.jobs?.scheduled || 0) + (statsData?.jobs?.printing || 0)}</div>
            <div className="text-sm text-farm-400">Active Jobs</div>
          </div>
          <div className="bg-farm-800 rounded-lg p-4">
            <div className="text-2xl font-bold">{statsData?.jobs?.completed || 0}</div>
            <div className="text-sm text-farm-400">Completed</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          className="flex items-center gap-2 px-6 py-2 bg-print-600 hover:bg-print-500 disabled:opacity-50 rounded-lg font-medium"
        >
