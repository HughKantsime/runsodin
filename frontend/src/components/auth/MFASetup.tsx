...
export default function MFASetup() {
  ...
  return (
    <div>
      ...
      {success && (
        <div className="mb-4 p-3 bg-green-900/30 border border-green-700 rounded-md flex items-center gap-2 text-sm text-green-400">
      ...
      {mfaEnabled && !setupData ? (
        ...
        <div>
          ...
          <form onSubmit={handleDisable} className="flex items-end gap-3">
            <div className="flex-1">
              <label className="block text-sm text-[var(--brand-text-secondary)] mb-1">Enter code to disable MFA</label>
              <input
                type="text"
                inputMode="numeric"
                maxLength={6}
                ...
                className="w-full bg-[var(--brand-input-bg)] border border-[var(--brand-card-border)] rounded px-3 py-2 text-sm font-mono tracking-widest"
                placeholder="000000"
              />
            </div>
            <button
        ...
        <div>
          ...
          <div className="flex flex-col items-center gap-4 mb-6">
            <img src={setupData.qr_code} alt="MFA QR Code" className="w-48 h-48 rounded-md bg-white p-2" />
            <div className="flex items-center gap-2">
              <code className="text-xs bg-[var(--brand-input-bg)] px-3 py-1.5 rounded font-mono text-[var(--brand-text-secondary)] select-all">
                {setupData.secret}
              </code>
              <button onClick={copySecret} className="p-1.5 rounded hover:bg-[var(--brand-input-bg)] text-[var(--brand-text-secondary)]">
          ...
          <form onSubmit={handleConfirm} className="flex items-end gap-3">
            <div className="flex-1">
              <label className="block text-sm text-[var(--brand-text-secondary)] mb-1">Verification code</label>
              <input
                type="text"
                inputMode="numeric"
                maxLength={6}
