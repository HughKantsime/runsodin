...
// TODO: type these properly when the filament/spool API types are finalized
interface FilamentSlotLike {
  id: number
  slot_number: number
...
interface SlotUpdateData {
  ...
  assigned_spool_id?: number
  spoolman_spool_id?: number | null
}

interface FilamentSlotEditorProps {
  slot: FilamentSlotLike
  allFilaments: FilamentOption[] | null
  spools: SpoolOption[] | null
  printerId: number
  onSave: (data: SlotUpdateData) => void
}

export default function FilamentSlotEditor({ slot, allFilaments, spools, printerId, onSave }: FilamentSlotEditorProps) {
  ...
  const handleSelectSpool = async (spool: SpoolOption) => {
    ...
    onSave({
      ...
      filament_type: spool.filament_material || undefined,
  ...
  const handleSelect = (filament: FilamentOption) => {
    onSave({
      color: `${filament.brand} ${filament.name}`,
      color_hex: filament.color_hex,
      filament_type: filament.material,
      spoolman_spool_id: filament.source === 'spoolman' ? parseInt(String(filament.id).replace('spool_', '')) : null
    })
    setIsEditing(false)
    setSearch('')
  }
