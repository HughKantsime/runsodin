  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--brand-primary)] hover:bg-[var(--brand-primary)] rounded-md text-sm">
      ...
      {showCreate && (
        <div className="mb-4 p-4 bg-[var(--brand-input-bg)] rounded-md border border-[var(--brand-card-border)] space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-[var(--brand-text-primary)]">New Report Schedule</span>
            <button onClick={() => setShowCreate(false)} className="p-1 text-[var(--brand-text-secondary)] hover:text-[var(--brand-text-primary)]"><X size={14} /></button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="rs-name" className="text-xs text-[var(--brand-text-secondary)] block mb-1">Name</label>
              <input id="rs-name" type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder="Weekly Jobs Summary" className="w-full bg-[var(--brand-card-bg)] border border-[var(--brand-card-border)] rounded-md px-3 py-2 text-sm" />
            </div>
            <div>
              <label htmlFor="rs-type" className="text-xs text-[var(--brand-text-secondary)] block mb-1">Report Type</label>
            ...
            <div>
              <label htmlFor="rs-freq" className="text-xs text-[var(--brand-text-secondary)] block mb-1">Frequency</label>
            ...
            <div>
              <label htmlFor="rs-recipients" className="text-xs text-[var(--brand-text-secondary)] block mb-1">Recipients (comma-separated emails)</label>
              <input id="rs-recipients" type="text" value={form.recipients} onChange={e => setForm(f => ({ ...f, recipients: e.target.value }))}
                placeholder="admin@example.com" className="w-full bg-[var(--brand-card-bg)] border border-[var(--brand-card-border)] rounded-md px-3 py-2 text-sm" />
            </div>
          </div>
          <button onClick={handleCreate} disabled={!form.name.trim() || createSchedule.isPending}
            aria-busy={createSchedule.isPending}
            className="px-4 py-2 bg-[var(--brand-primary)] hover:bg-[var(--brand-primary)] disabled:opacity-50 rounded-md text-sm">
      ...
      {!isLoading && (!schedules || schedules.length === 0) && (
        <p className="text-sm text-[var(--brand-text-muted)] py-4">No scheduled reports configured.</p>
      )}

      {schedules?.map(s => (
        <div key={s.id} className="flex items-center justify-between p-3 bg-[var(--brand-input-bg)] rounded-md mb-2 border border-[var(--brand-card-border)]">
          <div className="min-w-0 flex-1">
            <div className="text-sm font-medium text-[var(--brand-text-primary)]">{s.name}</div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className="p-1.5 text-[var(--brand-text-muted)] hover:text-[var(--brand-primary)] rounded-md disabled:opacity-50"
              title="Send now"
            >
              <Zap size={14} />
            </button>
            <button
              ...
              className={`p-1.5 rounded-md ${s.is_active ? 'text-green-400 hover:bg-green-900/30' : 'text-[var(--brand-text-muted)] hover:bg-[var(--brand-input-bg)]'}`}
            ...
            <button
              ...
              className="p-1.5 text-[var(--brand-text-muted)] hover:text-red-400 rounded-md"
