...
import { Printer as PrinterIcon, Package, QrCode, Scale, Archive, Pencil, Droplets } from 'lucide-react'
import { canDo } from '../../permissions'
import { ProgressBar, Button, SpoolRing } from '../ui'
import type { Spool, Printer } from '../../types'
...
interface SpoolCardProps {
  ...
  onUnload: (spool: Spool) => void
  onUse: (spool: Spool) => void
  onArchive: (spool: Spool) => void
  onEdit: (spool: Spool) => void
  onDry: (spool: Spool) => void
  printers?: Printer[]
}
export default function SpoolCard({ spool, onLoad, onUnload, onUse, onArchive, onEdit, onDry, printers }: SpoolCardProps) {
    ...
    >
      ...
      <div className="flex gap-1.5 md:gap-2 flex-wrap">
        {canDo('spools.edit') && spool.location_printer_id ? (
          ...
          <Button variant="primary" size="sm" icon={PrinterIcon} onClick={() => onLoad(spool)} title="Load into printer">
